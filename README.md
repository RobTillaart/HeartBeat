...
Added callback function, see **onStateChange()**.
...
With version 0.4.0 the HearBeat classes can register a callback function
that is triggered when the state changes.

- **void onStateChange(HeartBeatCallback callback)**

The function has the following fignature ```void (*)(uint8_t state)```
so the callback function gets the new state as parameter.
It is not allowed to call **beat()** in the callback function.
Best practice is to keep the callback function as short as possible.

The callback function will be deregistered by calling **onStateChange()**
with no parameters, or calling it with a new callback function.
To enable the callback function again one need to reregister it.
...
void hbStateChanged(uint8_t newstate)
{
  if (newstate == HIGH)
  {
    //  ...
...
- need a getter on lastNotifiedState?
